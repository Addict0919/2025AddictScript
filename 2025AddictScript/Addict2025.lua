...
local holding_entity = nil
local auto_mode = false

-- Get entity aimed at (vehicle, ped, object)
local function get_target_entity()
    local outEntity = memory.alloc_int()
    if PLAYER.GET_ENTITY_PLAYER_IS_FREE_AIMING_AT(PLAYER.PLAYER_ID(), outEntity) then
        local entity = memory.read_int(outEntity)
        if ENTITY.DOES_ENTITY_EXIST(entity) and ENTITY.IS_ENTITY_AN_OBJECT(entity) or ENTITY.IS_ENTITY_A_PED(entity) or ENTITY.IS_ENTITY_A_VEHICLE(entity) then
            return entity
        end
    end
    return 0
...
-- Attach the target to player's hand
local function pick_up_entity()
    if holding_entity ~= nil then
        util.toast("Already holding something.")
        return
    end

    local entity = get_target_entity()
    if entity == 0 then
        util.toast("No valid entity targeted.")
        return
    end

    local ped = PLAYER.PLAYER_PED_ID()
    local bone = PED.GET_PED_BONE_INDEX(ped, 57005) -- right hand
    ENTITY.ATTACH_ENTITY_TO_ENTITY(entity, ped, bone, 0.0, 0.0, 1.3, 0.0, 0.0, 0.0, true, true, false, false, 2, true)
    ENTITY.SET_ENTITY_COLLISION(entity, false, true)
    ENTITY.SET_ENTITY_INVINCIBLE(entity, true)

    holding_entity = entity
    util.toast("Picked it up!")
end

-- Throw the held entity
local function throw_entity()
    if holding_entity == nil then
        util.toast("You're not holding anything.")
        return
    end

    local ped = PLAYER.PLAYER_PED_ID()
    local forward = ENTITY.GET_ENTITY_FORWARD_VECTOR(ped)

    ENTITY.DETACH_ENTITY(holding_entity, true, true)
    ENTITY.APPLY_FORCE_TO_ENTITY_CENTER_OF_MASS(
        holding_entity, 1, forward.x * 80.0, forward.y * 80.0, 15.0,
        false, true, true, false
    )

    ENTITY.SET_ENTITY_COLLISION(holding_entity, true, true)
    ENTITY.SET_ENTITY_INVINCIBLE(holding_entity, false)

    holding_entity = nil
    util.toast("Yeeted it!")
end

-- Auto pickup logic (runs in a loop)
local function auto_pickup_loop()
    while auto_mode do
        if holding_entity == nil then
            local entity = get_target_entity()
            if entity ~= 0 then
                local ped = PLAYER.PLAYER_PED_ID()
                local bone = PED.GET_PED_BONE_INDEX(ped, 57005)
                ENTITY.ATTACH_ENTITY_TO_ENTITY(entity, ped, bone, 0.0, 0.0, 1.3, 0.0, 0.0, 0.0, true, true, false, false, 2, true)
                ENTITY.SET_ENTITY_COLLISION(entity, false, true)
                ENTITY.SET_ENTITY_INVINCIBLE(entity, true)
                holding_entity = entity
                util.toast("Auto-picked something up!")
            end
        end
        util.yield(200)
    end
...
--[[
Pick_up_entitys = addict.list(Self_addict, "Pick Up Entitys", {}, "Not finished", function() end)

menu.action(Pick_up_entitys, "Pick Up Entity", {"pickup"}, "Aim at a vehicle, ped, or object and pick it up.", pick_up_entity)
menu.action(Pick_up_entitys, "Throw Entity", {"throw"}, "Throw the held object or person.", throw_entity)

menu.toggle(Pick_up_entitys, "Auto Mode", {"autopick"}, "Automatically picks up whatever you aim at.", function(enabled)
    auto_mode = enabled
    if enabled then
        auto_pickup_loop()
    end
 ...
 ------------------------------------------------------------------------------------------------------------------------------------------
    ...
    addict.toggle_loop(Self_addict, "Accept Joins & Transaction Errors!", {"accepterrors"}, "Automatically accept join screens and transaction errors.", function()
    ...
    --[[
    addict.toggle_loop(Self_addict, "Accept Joins", {"autojoin"}, "Automatically accept join screens reliably.", function()
    ...
    addict.toggle_loop(Self_addict, "Auto Host", {"autohost"}, "Great for basic and regular users.", function() 
    ...
    addict.toggle_loop(Self_addict, "Auto Enter Key", {"autoenter"}, "Automatically hits enter key.", function()
    ...
    addict.slider_float(Self_addict, "God Graphics Level", {"godgraphlvl"}, "", 1, 1000, 125, 1, function(s)
        god_graphics_level = s * 0.001
    end)


    menu.action(Self_addict, "Apply God Graphics", {"godgraphon"}, "", function(click_type)
        addict.trigger_commands("shader intnofog")
        addict.trigger_commands("lodscale " .. god_graphics_level)
    end)

    addict.action(Self_addict, "Unapply God Graphics", {"godgraphoff"}, "", function(click_type)
        addict.trigger_commands("shader off")
        addict.trigger_commands("lodscale 1.00")
    end)

        addict.toggle(Self_addict, "Drive Cop Heli", {"copheli"}, "Plus bodygaurds.", function(on_toggle)
            if on_toggle then
        ...
        addict.toggle(Self_addict, "Drive Cop Car", {"copcar"}, "Plus a bodygaurd.", function(on_toggle)
            if on_toggle then
...
local ride_menu = addict.list(Self_addict, "Pig Rider", {}, "Ride and control a pig like a mount.")
...
local ride_menu = addict.list(Self_addict, "Cow Rider", {}, "Ride and control a cow like a mount.")
...
addict.divider(worldchaos, "World Chaos")
    ...
    addict.toggle_loop(worldchaos, "Explode Map", {""}, "", function()
...
menu.toggle_loop(resellspam, "Re Seller Chat Spam", {}, "Spams every 2.5 minutes", function(on_toggle)
chat.send_message(
...
addict.toggle_loop(Protex,"Clear Shit Up Toggled", {"togglesuperc"}, "Toggled Clean up.", function()
    menu.trigger_commands("superc 1")
end)
...
menu.toggle_loop(Protex, "Suspicious Ped Deletion", {}, "Deletes peds with invalid models or crash-like traits", function()
...
menu.toggle_loop(Protex, "Name Spoof Detection", {}, "Warns you if a player joins with a name similar to yours", function()
...
menu.action(Protex, "Force Cleanup Entities", {}, "Deletes unowned/unused entities", function()
...
-- CONFIG MENU
menu.slider(British_Attack, "Number of Attackers", {"ukcount"}, "", 1, 48, config.count, 1, function(val)
    config.count = val
end)
...
menu.list_select(British_Attack, "Select Weapon", {"ukweapon"}, "Choose a weapon from the list", weapon_names, get_weapon_index(config.weaponslist), function(index, value)
    config.weaponslist = value
end)


-- MAIN TOGGLE
menu.action(British_Attack, "Spawn UK Attackers", {"ukattack"}, "Spawns hostile English Dave attackers with flags", function()
    local p = PLAYER.GET_PLAYER_PED_SCRIPT_INDEX(pid)
...
-- Cleanup: 
menu.action(AttackerPeds, "Clear Nearby Attackers", {"clearattacks"}, "Clears all IG_EnglishDave_02 peds near you", function()
    addict.trigger_commands("superc 5")
    util.yield(500)
...
menu.action(Five_G_Crashes, "5g Crashv3", {"fivegv3"}, "Breaks traffic lights and lamp posts around the player then spawns towtrucks and attaches them to lamp post.", function()
