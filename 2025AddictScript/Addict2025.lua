...
addict.hyperlink(github, "Change Log", "https://github.com/Addict0919/2025AddictScript/blob/main/2025AddictScript/AddictScriptChangelog.LUA")
...
-------------------------------------------------------------------------------------------------------------------------
-- Targeted Jacker
-- Spawns a ped who jacks the nearest vehicle to the target player and drives it into the ocean
...
-------------------------------------------------------------------------------------------------------------------------
-- Sky Text Functions
-- Store spawned vehicles for cleanup
...
-- Freeze a vehicle and make it invincible
local function freeze_vehicle(veh, state)
    ENTITY.FREEZE_ENTITY_POSITION(veh, state)
    ENTITY.SET_ENTITY_INVINCIBLE(veh, true)
end

-- Spawn and freeze a vehicle at given coordinates
...
-- Draw given text in the sky using vehicles
...
-- Cleanup all spawned sky vehicles
local function cleanupSkyVehicles()
...
addict.toggle(ride_menu, "Ride a Surfboard", {"ridesurf"}, "Spawn and control a surfboard on water.", function(on)
    ...
    if on then
        ...
        -- Spawn invisible support object
        ...
        -- Spawn surfboard and attach to support
...
addict.toggle(ride_menu, "Ride a Skateboard", {"rideskate"}, "Spawn and control a skateboard. Note: You must move forward to stop the floating upwards unti I can figure out a fix.", function(on)
    ...
    if on then
        ...
        -- Spawn support object slightly under player
        ...
        -- Spawn skateboard prop rotated 90° relative to support
        ...
        riding = true
        util.toast("You are now riding and controlling a skateboard!")
...
addict.slider(ride_menu, "Skateboard Speed", {"skatespeed"}, "Adjust your skateboard speed", 1, 100, 1, 1, function(val)
    speed = val * 0.1 -- scales integer slider to desired speed
end)
...
-----------------------------------------------------------------------------------------------------------------------------------------
    ...
    addict.toggle_loop(Self_addict, "Tp all over Map V1", {"tpspread1"}, "Note: Great for spreading any kinda mods like gifting vehicles or crash events.", function()
        ...
        --ENTITY.SET_ENTITY_COORDS_NO_OFFSET(p, c.x + math.random(0, 2555), c.y + math.random(0, 2815), math.random(1, 1232), height) 
...
-- Tables to track props and protected vehicles
local attached_props = {}
local protected_vehicles = {}

-- Add text input and sliders to the menu
...
-- Helper: safe vehicle protection
...
local function detach_all_props()
    ...
    -- Release protected vehicles safely
...
-- Action: attach objects to nearby vehicles
addict.action(Vehicle_2, "Attach Object to Nearby Vehicles", {}, "", function()
    ...
    for _, vehicle in ipairs(vehicles) do
        ...
        -- Attach to vehicle with offsets & yaw
        ENTITY.ATTACH_ENTITY_TO_ENTITY(
            obj, vehicle, 0,
        ...
        -- Protect vehicle if toggle is on
...
local function applyMagnetForce(baseVeh, targetEnt, polarity)
    ...
    -- Force magnitude inversely proportional to distance, scaled by polarity
    local forceMag = (polarity * 50) / dist

    -- Apply force on the entity (relative to itself)
...
local function magnetLoop()
    while magnetOn do
        ...
        if ENTITY.DOES_ENTITY_EXIST(playerVeh) then
            ...
            -- Attract or repel peds (except player)
...
addict.action(HitchhikerChaos, "Launch Next Hitchhiker", {}, "Launch one attached ped forward each time you press", function()
    ...
    -- Wrap around launchIndex if it goes beyond list size
    ...
    -- Remove ped from table so they aren’t launched twice
    table.remove(attachedPeds, launchIndex)

    util.toast("Launched hitchhiker #" .. launchIndex)

    -- Don't increment launchIndex since we removed current ped; next ped shifts into this index
end)

-- Reset all attached hitchhikers: detach and clear list
...
addict.action(StealEngines, "Open/Close Hood", {}, "Open or close the hood of the nearest vehicle", function()
    ...
    -- Get current hood state (0 = closed, 1 = open)
...
-- Find closest vehicle within a given distance
...
-- Steal Engine
addict.action(StealEngines, "Steal Engine", {}, "Pull engine from nearest vehicle and carry it", function()
    local ped = players.user_ped()
...
addict.action(StealEngines, "Steal All 4 Tyres", {}, "Pull all tyres off nearest vehicle and carry them stacked", function()
    ...
    -- Burst all 4 tyres so car sinks on rims
    ...
    -- Spawn position a little away from the vehicle to avoid collisions/deletion
    ...
    util.yield(5000)
    -- Freeze vehicle to simulate wheel removal (optional)
    ENTITY.FREEZE_ENTITY_POSITION(closestVeh, true)
...
addict.action(StealEngines, "Steal All 4 Wheels", {}, "Pull all wheels off nearest vehicle, remove them from the car, and carry as stack. Works best with 4x4 vehicles.", function()
    ...
    -- Remove the wheels from the actual vehicle
    native_invoker.begin_call()
    native_invoker.push_arg_int(closestVeh)
...
addict.action(StealEngines, "Clear All Parts", {}, "Delete all spawned parts", function()
    local objs = entities.get_all_objects_as_handles()
    for _, obj in ipairs(objs) do
        local model = ENTITY.GET_ENTITY_MODEL(obj)
        for _, partModel in ipairs(PART_MODELS) do
            if model == partModel then
    ...
    util.toast("All vehicle parts cleared!")
end)
...
-- Cleanup function to delete guitars
local function cleanup()
...
addict.toggle(worldchaos, "All Peds Play Guitars/Bongos", {"musicpeds"}, "Makes all nearby peds play guitars and bongos.", function(toggle)
    is_enabled = toggle
    if toggle then
        -- On toggle ON: apply once to all nearby peds
        for _, ped in ipairs(entities.get_all_peds_as_handles()) do
            if ped ~= PLAYER.PLAYER_PED_ID() and not PED.IS_PED_A_PLAYER(ped) then
                ...
                -- Guitar spawning and attaching removed as requested
            end
        end
    else
        -- On toggle OFF: cleanup guitars and clear ped tasks
        cleanup()
        for _, ped in ipairs(entities.get_all_peds_as_handles()) do
            if ped ~= PLAYER.PLAYER_PED_ID() and not PED.IS_PED_A_PLAYER(ped) then
                TASK.CLEAR_PED_TASKS_IMMEDIATELY(ped)
...
-- Selected object for live manipulation
local selected_index = nil

-- Default offsets and rotations
local offset_x = -15.0
local offset_y = 0.0
local offset_z = -01.0
local rot_x = 0.0
...
-- Safely request and wait for model load
...
local function spawn_new_object(model_name)
    ...
    -- Create object at player position + current offsets
...
-- Update selected object's position and rotation
...
local DeSync_Doctor_Root = Protex
...
local function respawn_here(invuln_ms)
    ...
    -- Arguments: x, y, z, heading, invulnTime, unk1 BOOL, unk2 BOOL, unk3 int, unk4 int
...
local DeSync_Doctor = addict.list(DeSync_Doctor_Root, "DeSync Doctor")

addict.action(DeSync_Doctor, "One-shot Resync", {}, "", one_shot_resync)
addict.action(DeSync_Doctor, "Safe Resync Profile", {}, "Freeze+Snap+Nudge+VehStabilise", safe_resync)
addict.action(DeSync_Doctor, "Respawn Here", {}, "", function() respawn_here(1200) end)
addict.action(DeSync_Doctor, "Re-seat Driver", {}, "", reseat_driver)
addict.action(DeSync_Doctor, "Snap to Ground", {}, "", snap_ped_to_ground)

addict.toggle(DeSync_Doctor, "Stuck Mid-Air Fix", {}, "", function(s) stuck_cfg.enabled = s end)
addict.toggle(DeSync_Doctor, "Parachute Snap", {}, "", function(s) parachute_cfg.enabled = s end)
addict.toggle(DeSync_Doctor, "Vehicle Pings", {}, "", function(s) ping_cfg.enabled = s end)
addict.slider(DeSync_Doctor, "Ping Radius", {}, "", 10,200,ping_cfg.radius,5,function(v) ping_cfg.radius=v end)
addict.slider(DeSync_Doctor, "Ping Interval", {}, "", 500,10000,ping_cfg.interval,100,function(v) ping_cfg.interval=v end)
addict.toggle(DeSync_Doctor, "Keepalive Sweep", {}, "", function(s) sweep_cfg.enabled = s end)

-- Whitelist / Blacklist
local wl = addict.list(DeSync_Doctor, "Whitelist", {""}, "For pings.")
local bl = addict.list(DeSync_Doctor, "Blacklist", {""}, "For pings.")
for _, pid in ipairs(players.list(true,true)) do
...
addict.list(addict.my_root(), "V0.07", {"addictversion"})
---------------------------------------------------------------------------------------------------------------------------------------
...
addict.action(StealEngine, "Steal Engine", {"stealeng"}, "Pull engine from nearest vehicle and carry it.", function()
    ...
    -- Spawn fake engine object at vehicle's front
...
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    addict.action(griefingtp, "TP Everyone To MazeBank", {"tpallmazebank"}, "Teleports all players to mazebank you.", function()
        excludeselected = true
        addict.trigger_commands("tpplayersmazebank")
    end, nil, nil, COMMANDPERM_AGGRESSIVE)
...
local heliAttachHeight = 1  -- default height offset (integer for slider)
...
-- Spawn helicopter and ped safely
...
-- Spawn and attach user vehicle to rotor
...
-- Cleanup all spawned entities and tick handler
local function cleanup()
...
-- Attach helicopter to player ped if enabled, using heliAttachHeight
local function attachHeliToPlayerPed()
    ...
    ENTITY.ATTACH_ENTITY_TO_ENTITY(
        heli, playerPed, 0,
        0.0, 0.0, heliAttachHeight,  -- use slider height here
        0.0, 0.0, 0.0,
        false, false, false, false, 2, true
    )
...
local function toggleHeliLink(enabled)
    if enabled then
        ...
        -- Tick to ragdoll and kill nearby peds
        util.create_tick_handler(function()
...
-- Slider for heli attach height (step must be integer)
...
-- Menu toggle for attaching the helicopter itself to player
...
-- Main toggle for spawning and attaching
...
-- Spawns helicopter with spinning blades, attaches scrap parts from nearby vehicles to player, and mounts heli on top
...
-- Attach nearby scrap parts and heli
local function attach_scrap_and_blade()
    local playerPed = PLAYER.GET_PLAYER_PED_SCRIPT_INDEX(pid)
    local playerPos = ENTITY.GET_ENTITY_COORDS(playerPed, true)

    -- Collect broken vehicle parts (objects)
    ...
    if heli_handle and ENTITY.DOES_ENTITY_EXIST(heli_handle) then
        ...
        -- Attach heli to scrap or player
...
----------------------------------------------------------------------------------------------------------------------------------------
    ...
    addict.action(LexisCrashes, "Lexis ATF Collapse 3 [Little Unstable]", {"lexis3"}, "Invalid CTaskReloadGun", function()
        local self_ped = PLAYER.GET_PLAYER_PED_SCRIPT_INDEX(players.user())
    ...
    addict.action(LexisCrashes, "Lexis Hand Collapse 4", {"lexis4"}, "Hand Speed (Skidded From China)", function()
    ...
    addict.action(LexisCrashes, "Lexis Sweep Collapse", {"lexis1"}, "Invalid CTaskGeneralSweep", function()
        local self_ped = PLAYER.GET_PLAYER_PED_SCRIPT_INDEX(players.user())
        local target_ped = PLAYER.GET_PLAYER_PED_SCRIPT_INDEX(PlayerID)
        local cord = ENTITY.GET_ENTITY_COORDS(target_ped, true)
...
addict.action(Friendly, "Send Friend Request", {"sendfriend"}, "", function()
	...
	Ped_Cash = addict.list(Friendly, "Ped Cash", {}, "", function(); end)
...
addict.toggle_loop(Friendly, "Drop Cash Loopv1", {"cashloop"}, "Less errors, 3600ms cash drops. Must be host. (NEEDS FIXING I THINK...)", function()
    local coords = players.get_position(pid)
    coords.z = coords.z + 1.5
