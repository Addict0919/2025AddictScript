...
-- Table to track attached props so we can remove them later
...
-- Function to detach all props spawned by this script
local function detach_all_props()
    ...
    attached_props = {}
    util.toast("Detached all attached props.")
end
...
addict.action(Vehicle_2, "Attach Object to Nearby Vehicles", {}, "", function()
    ...
    -- Detach previous props first to avoid duplicates
    detach_all_props()
...
---1054659037
---1054659196
addict.toggle_loop(Friendly, "Drop Cash Loopv1", {"cashloop"}, "Less errors, 3600ms cash drops. Must be host.", function()
    local coords = players.get_position(pid)
    coords.z = coords.z + 1.5
...
addict.toggle_loop(Friendly, "Drop Cash Loopv2", {"cashloop2"}, "Less errors, 3600ms cash drops. Must be host.", function()
    local coords = players.get_position(pid)
    coords.z = coords.z + 1.5
    local cash = MISC.GET_HASH_KEY("PICKUP_VEHICLE_MONEY_VARIABLE")
    STREAMING.REQUEST_MODEL(cash)
...
addict.toggle_loop(Friendly, "Drop Cash Loopv3", {"cashloop3"}, "Less errors, 3600ms cash drops. Must be host.", function()
